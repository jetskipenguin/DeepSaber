...
def update_generated_metadata(gen_folder: Path, beatmap_folder: Path, config: Config):
    ...
    # 1. Grab existing difficulty settings to copy over Note Jump Speed if available
    existing_diffs = {}
    if '_difficultyBeatmapSets' in info:
        for map_set in info['_difficultyBeatmapSets']:
            if map_set.get('_beatmapCharacteristicName') == 'Standard':
                for diff in map_set.get('_difficultyBeatmaps', []):
                    existing_diffs[diff.get('_difficulty')] = diff

    # 2. Find what was ACTUALLY generated by the model in the output folder
    generated_dats = [f for f in gen_folder.glob('*.dat') if f.name.lower() != 'info.dat']
    new_difficulties = []
    
    # Standard Beat Saber Rank Mappings
    rank_map = {'Easy': 1, 'Normal': 3, 'Hard': 5, 'Expert': 7, 'ExpertPlus': 9}
    
    for dat_file in generated_dats:
        diff_name = dat_file.stem  # e.g., "Expert"
        
        # Start with existing data if we have it, or provide a safe default
        diff_entry = existing_diffs.get(diff_name, {
            "_difficulty": diff_name,
            "_difficultyRank": rank_map.get(diff_name, 7),
            "_noteJumpMovementSpeed": 15,
            "_noteJumpStartBeatOffset": 0,
            "_customData": {}
        })
        
        # CRITICAL FIX: Force the filename to match the generated file EXACTLY
        diff_entry['_beatmapFilename'] = dat_file.name
        
        new_difficulties.append(diff_entry)

    # 3. Inject the corrected array back into the Standard characteristic set
    has_standard = False
    if '_difficultyBeatmapSets' not in info:
        info['_difficultyBeatmapSets'] = []
        
    for map_set in info['_difficultyBeatmapSets']:
        if map_set.get('_beatmapCharacteristicName') == 'Standard':
            map_set['_difficultyBeatmaps'] = new_difficulties
            has_standard = True
            break
    ...
    if not has_standard:
        info['_difficultyBeatmapSets'].append({
            ...
            "_difficultyBeatmaps": new_difficulties
        })

    # DeepSaber requires the BPM to be clamped to 60 for its math to work in-game
